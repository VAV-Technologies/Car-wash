...
interface Props {
  agentName: string
  /** Display name shown in the UI (e.g. "Shera", "Ryan"). Defaults to agentName. */
  displayName?: string
  /** Subheading shown below the "Custom Rules" header. */
  description?: string
  /** Placeholder text for the rule-content textarea. */
...
export default function AgentRulesEditor({
  ...
  const subheading =
    description ||
    `Rules you add here get injected into ${label}'s context on every run. Use them to override behavior, add constraints, or give specific instructions.`
  const placeholder =
    contentPlaceholder ||
    `Rule content — be specific about what ${label} should or shouldn't do...`
      ...
      <div className="flex items-start justify-between gap-4">
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-white/40 text-sm mt-0.5">{subheading}</p>
          </div>
        </div>
      ...
      {!loading && rules.length === 0 && (
        <div className="text-center py-12">
          ...
          <p className="text-white/25 text-xs mt-1">
            Add rules to customize how {label} responds.
          </p>
        </div>
      )}
